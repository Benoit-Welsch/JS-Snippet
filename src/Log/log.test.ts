import { describe, it, expect, afterAll, beforeAll, beforeEach, afterEach } from 'bun:test';
...
describe('log', () => {
  ...
  beforeEach(() => {
    try { fs.rmdirSync('./logs', { recursive: true }) }
    catch (e) { }
  ...
  it('should log to a File', async () => {
    const logger = new Logger(
      { t: [new File({ path: './logs/random.txt' })] }
    );
